...
import "../token/RUPXTokenInterface.sol";


contract RUPXRebaser {
     ...
     * @notice Sets the reserve contract
     */
    event TreasuryIncreased(uint256 reservesAdded, uint256 rupxsSold, uint256 rupxsFromReserves, uint256 rupxsToReserves);
    ...
    /// @notice RUPX token address
    address public rupxAddress;
    ...
    /// @notice pair for reserveToken <> RUPX
    ...
    /// @notice Whether or not this token is first in uniswap RUPX<>Reserve pair
    bool public isToken0;

    constructor(
        address rupxAddress_,
        ...
        public
    {
          ...
          (address token0, address token1) = sortTokens(rupxAddress_, reserveToken_);

          // used for interacting with uniswap
          if (token0 == rupxAddress_) {
              isToken0 = true;
          } else {
              isToken0 = false;
          }
          // uniswap RUPX<>Reserve pair
    ...
    */
        ...
        public
    {
        ...
        RUPXTokenInterface rupx = RUPXTokenInterface(rupxAddress);

        if (positive) {
            require(rupx.rupxsScalingFactor().mul(uint256(10**18).add(indexDelta)).div(10**18) < rupx.maxScalingFactor(), "new scaling factor will be too big");
        }


        uint256 currSupply = rupx.totalSupply();
        ...
        // rebase
        uint256 supplyAfterRebase = rupx.rebase(epoch, indexDelta, positive);
        assert(rupx.rupxsScalingFactor() <= rupx.maxScalingFactor());
        ...
        public
    {
        ...
        if (uniVars.amountFromReserves > 0) {
            ...
            rupx.transferFrom(reservesContract, uniswap_pair, uniVars.amountFromReserves);
            if (uniVars.amountFromReserves < uniVars.rupxsToUni) {
                // if the amount from reserves > rupxsToUni, we have fully paid for the yCRV tokens
                // thus this number would be 0 so no need to mint
                rupx.mint(uniswap_pair, uniVars.rupxsToUni.sub(uniVars.amountFromReserves));
            }
        } else {
            // mint to uniswap
            rupx.mint(uniswap_pair, uniVars.rupxsToUni);
        }

        // mint unsold to mintAmount
        if (uniVars.mintToReserves > 0) {
            rupx.mint(reservesContract, uniVars.mintToReserves);
        }

        // transfer reserve token to reserves
        if (isToken0) {
            SafeERC20.safeTransfer(IERC20(reserveToken), reservesContract, amount1);
            emit TreasuryIncreased(amount1, uniVars.rupxsToUni, uniVars.amountFromReserves, uniVars.mintToReserves);
        } else {
            SafeERC20.safeTransfer(IERC20(reserveToken), reservesContract, amount0);
            emit TreasuryIncreased(amount0, uniVars.rupxsToUni, uniVars.amountFromReserves, uniVars.mintToReserves);
        }
    }
        ...
        internal
    {
        ...
        // check if protocol has excess rupx in the reserve
        uint256 excess = rupx.balanceOf(reservesContract);


        uint256 tokens_to_max_slippage = uniswapMaxSlippage(token0Reserves, token1Reserves, offPegPerc);

        UniVars memory uniVars = UniVars({
          rupxsToUni: tokens_to_max_slippage, // how many rupxs uniswap needs
          amountFromReserves: excess, // how much of rupxsToUni comes from reserves
          mintToReserves: 0 // how much rupxs protocol mints to reserves
        });
        ...
        if (isToken0) {
            if (tokens_to_max_slippage > mintAmount.add(excess)) {
                ...
                uniVars.rupxsToUni = mintAmount + excess;
                ...
                if (tokens_to_max_slippage > excess) {
                    ...
                    // swap up to slippage limit, taking entire rupx reserves, and minting part of total
