...
// Modified to work in the RUPX system

// all votes work on underlying _rupxBalances[address], not balanceOf(address)
...
//
// Changes made by RUPX after audit:
//    Formatting, naming, & uint256 instead of uint
//    Since RUPX supply changes, updated quorum & proposal requirements
//    If any uint96, changed to uint256 to match RUPX as opposed to comp
...
contract GovernorAlpha {
    ...
    string public constant name = "RUPX Governor Alpha";

    /// @notice The number of votes in support of a proposal required in order for a quorum to be reached and for a vote to succeed
    function quorumVotes() public view returns (uint256) { return SafeMath.div(SafeMath.mul(rupx.initSupply(), 4), 100); } // 4% of RUPX

    /// @notice The number of votes required in order for a voter to become a proposer
    function proposalThreshold() public view returns (uint256) { return SafeMath.div(rupx.initSupply(), 100); } // 1% of RUPX
    ...
    constructor(address timelock_, address rupx_) public {
        timelock = TimelockInterface(timelock_);
        rupx = RUPXInterface(rupx_);
        guardian = msg.sender;
    }
    ...
    function propose(
        ...
        returns (uint256)
    {
        require(rupx.getPriorVotes(msg.sender, sub256(block.number, 1)) > proposalThreshold(), "GovernorAlpha::propose: proposer votes below proposal threshold");
        ...
        public
    {
        ...
        Proposal storage proposal = proposals[proposalId];
        require(msg.sender == guardian || rupx.getPriorVotes(proposal.proposer, sub256(block.number, 1)) < proposalThreshold(), "GovernorAlpha::cancel: proposer above threshold");
        ...
        internal
    {
        ...
        uint256 votes = rupx.getPriorVotes(voter, proposal.startBlock);
